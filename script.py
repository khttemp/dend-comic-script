...
index = 0
slowFlag = False
print("DEND COMIC SCRIPT ver1.2.0...")
name = input("comicのbinファイル名を入力してください: ")

try:
    if os.path.exists(name) == False:
        print("指定されたファイルが見つかりません。終了します。")
        input()
        sys.exit()
    f = open(name, "rb")
    line = f.read()
    print("見つけました！")
    size = len(line)
    result = input("ReadComicDataを１行ずつ読みますか？(Y/N)")
    ...
    while True:
        if result == "Y" or result == "y":
            ...
            result = input("入力エラー！改めて入力してください：")

    index += 16
    buf = line[0:16]
    if buf != b"DEND_COMICSCRIPT":
        print("DEND comic scriptファイルではありません。終了します。")
        input()
        sys.exit()
    index += 1
    ...
    for i in range(b):
        ...
        for j in range(4):
            ...
            print("{0}, ".format(struct.unpack("<f", text)[0]), end="")
            index += 4
        print("]")
    print()
    ...
    for i in range(secnt):
        ...
        index += 1
        text = line[index:index+b].decode()
        print("{0} -> {1}".format(i, text), end="")
        index += b
        print(", {0}".format(line[index]))
        index += 1
    print()
    ...
    for i in range(num):
        if index >= size:
            print("注意！設定したコマンド数({0})は、書き込んだコマンド数({1})より多く読もうとしています".format(num, i))
            input()
            sys.exit()

        print("No.{0} -> index({1})".format(i, hex(index)))
        num2 = struct.unpack("<H", line[index:index+2])[0]
        if num2 < 0 or num2 >= len(cmdList):
            print("定義されてないコマンド番号です({0})。読込を終了します。".format(num2))
            input()
            sys.exit()
        print("cmd -> {0}({1})".format(cmdList[num2], num2))
        index += 2
        b = line[index]
        if b >= 16:
            print("scriptErr!!")
        ...
        print("cmd_param -> [", end="")
        for j in range(b):
            print("{0}, ".format(struct.unpack("<f", line[index:index+4])[0]), end="")
            index += 4
        print("]")
    ...
    if index < size:
        print("注意！設定したコマンド数({0})は、書き込んだコマンド数より少なく設定されています".format(num))
        input()
        sys.exit()
    elif index == size:
